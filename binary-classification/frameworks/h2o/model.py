...
for DATASET_NAME in all_datasets_ls:
    ...
    for count, (train_idx, test_idx) in enumerate(skf.split(X, y)):
        ...
        x_col = htrain.columns
        y_col = "target"
        x_col.remove(y_col)

        # For binary classification, response should be a factor
        htrain[y_col] = htrain[y_col].asfactor()

        automl = H2OAutoML(max_runtime_secs=TIME_LIMIT, 
                   seed=RANDOM_SEED, 
                   stopping_metric='AUC', 
                   sort_metric='AUC', 
        ...
        automl.train(x=x_col, y=y_col, training_frame=htrain)
        lb = automl.leaderboard
        print (lb)
        # The leader model is stored here
        #automl.leader
        # PREDICT
        predictions = automl.leader.predict(htest)
        predictions = predictions.as_data_frame()
        y_test_predict_proba = predictions['p1']
        y_test_predict = predictions['predict']
